# 💡 키오스크 기능
  ...
  ### 출력
  - 키오스크가 제공하는 메뉴를 선택하고, 장바구니에 담을 수 있다.
      - 키오스크의 메뉴는 크게 카테고리(메뉴)와 메뉴아이템으로 나뉘며, 카테고리(버거, 치킨, 사이드)가 메뉴아이템들을 관리하고 출력한다.
  - 선택한 메뉴는 장바구니에 누적된다.
  - 누적된 장바구니 리스트들은 추가/삭제/수정이 가능하다.
  - 사용자는 장바구니 목록을 구매할 수 있다.
...
#### ✍ 고려한 점   
 ✅1단계의 경우는 계산기 과제 복습의 의도가 보여서 기본적인 기능을 구현했다. (2025.03.07)
 
 ✅사용자 클래스는 따로 정의하지않았고, main함수 바깥에서 선언되는 함수도없었다.(2025.03.07)
...
#### ✍ 고려한 점   
✅요구 사항에 맞게 구현 전에 미리 눈으로 보기 쉽게 구조 제작.(추후 나머지 모든 단계에서도 구조 제작) (2025.03.07)

✅MenuItem 클래스에는 `이름, 단품가격, 세트가격, 설명` 필드를 갖고 모두 `private`으로 선언해서 	`getter`, `setter`준비해줬다. (2025.03.07)
...
✅ main함수에서 키오스크 객체와 `List<MenuItem>`을 만들어주고, 키오스크 생성자에 해당 리스트를 넘기고 키오스크 시작 함수를 하면서 메인함수의 역할은 끝이난다. (2025.03.07)

✅ 이제부터 메인함수의 역할은 크게 없어질 것이다. (2025.03.07)

✅ 레벨 2와 별다른 기능적인 차이는 없다.(리팩토링 개념) (2025.03.07)
...
#### ✍ 고려한 점   
✅ 단계 만들 때 클래스 내부변수는 private해주고 게터와 세터를 만들어서 5단계를 동시에 만족해버렸다. 😅 (2025.03.07)

✅ 메뉴 카테고리 이름을 반환하는 메서드같은 경우는 menu에 카테고리 이름과 카테고리 번호를 저장하는 변수가 있다.  (2025.03.07)
  -> 출력할 때, 혹은 분류할 때 대분류/소분류 나누는 것 처럼 넘버링을 하면 관리가 쉽지 않을까 하는 생각으로 지정해주었음.

✅ 각각의 Menu객체들이 주어진 카테고리에 맞는 `List<MenuItem>`을 하나씩 들고 Menu리스트에 포함이 된다. (2025.03.07)
  ->이 Menu리스트들이 키오스크 클래스의 생성자로 들어가는 형식이다.

✅ 카테고리에 맞는 메뉴 보여주는 함수도 Menu에 넣었다. (2025.03.07)

✅ Kiosk의 kioskStart() 함수안에서 작동하던 로직들을 분리해서 private함수로 선언해주었다. (2025.03.07)
...
#### ✍ 고려한 점   
✅ MenuItem과 비슷한 형식의 장바구니를 만들어주고, 장바구니 데이터를 관리하기 위해 가격, 물품개수, 물품을 저장해줄 리스트 생성 (2025.03.10)

✅ 리스트가 비었는지 확인하면서 추가 출력 여부를 추가하고, 출력하는 함수도 가진다. (2025.03.10)

✅ 장바구니 내용을 거쳐서 결제가 가능하기 때문에 중간에 출력하는 함수를 생성해준다. (2025.03.10)
...
#### ✍ 고려한 점  
✅ 키오스크가 하는 일이 너무많아서 일부 분리를 위해 사용자 입력을 받고 입력 형태를 검증하는 클래스를 생성.  (2025.03.10)
...
✅ Enum같은 경우는 할인 목록 선택에 따라서 타입을 정해준다.  (2025.03.10)

✅ Enum 클래스의 discountPrice 함수를abstract int 형태로 선언했다가 함수형인터페이스를 이용해서 람다형식으로 리팩토링 해주었음 (둘 다 연습했다!)  (2025.03.12)

✅ 쇼핑카트 클래스에서 total금액을 조정하는 함수를 추가했다. (2025.03.12)

✅ 목록을 프린트 하는 경우 람다와 스트림을 활용해서 출력한다. (2025.03.12)
